...
void probar_agregar_alfinal(Lista* lista,void* cosa){
	printf("%p\n",cosa);
	pa2m_afirmar(lista_agregar_al_final(lista,cosa),"se agrego %p al final\n",cosa);
...
void probar_iterar(Lista* lista){
	...
	pa2m_afirmar(iterados==lista_cantidad_elementos(lista),"pokemons iterados correctamente");
}
...
void probar_agregar_en_pos(Lista* lista,void* cosa){
	printf("%p\n",cosa);
	size_t pos=2;
	pa2m_afirmar(lista_agregar_elemento(lista,pos,cosa),"se agrego %p en %zu\n",cosa,pos);
	printf("cant elem %zu\n",lista_cantidad_elementos(lista));
}
